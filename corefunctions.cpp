...
GLfloat * polygon_combine(GLfloat polygon1[][3], const GLuint num1, GLfloat polygon2[][3], const GLuint num2, GLuint * length){
    ...
    //接下去进行正式对各种情况(1)/(2)/(3)/(4)进行判断：
    
    //如果是情况(1):参考报告中2.3——2.5.1部分
    if(is_case_1(polygon1, num1, polygon2, num2)){
        
        //2.3先求出m,n(位于ploygon2),p,q(位于ploygon1).
        for(i = 0; i<num2 ; i++){
...
GLfloat * polygon_substract(GLfloat polygon1[][3], const GLuint num1, GLfloat polygon2[][3], const GLuint num2, GLuint * length){
     ...
     *length = k;
     ...
     if(index == -2) {
     //2.2,情况(3）
     //(3) polygon2无顶点位于polygon1内部，但 polygon1有顶点位于polygon2内部
     //
     for(k = 0;k < num2;k++){
     for(s = 0;s<num1;s++){//如果线段有交点，则记录
     if(has_intersection(polygon2[k], polygon1[s], polygon2[(k+1<num2)?k+1:0], polygon1[(s+1<num1)?s+1:0])){
     
     }
     }
     }
     for(int p = 0; p < num1;p++){
     newpolygon_vertices[p*3+0] = polygon1[p][0];
     newpolygon_vertices[p*3+1] = polygon1[p][1];
     newpolygon_vertices[p*3+2] = polygon1[p][2];
     }
