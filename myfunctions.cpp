            ...
            if( ( !is_in_polygon(polygon1, num1, polygon2[i]) )
               &&( !is_in_array(r, array_length, i) ) ){
                ...
                if(r[p+1]!= i){
                    ...
                    //要先判断接下去两个点的顺序：
                    //新的2个点和i-1点的三角形顺序即可：newpolygon[j]、p、p+1必定是逆时针的，否则就交换p和p+1的顺序
...
GLfloat * deal_case2_combine(GLfloat polygon1[][3], const GLuint num1, GLfloat polygon2[][3], const GLuint num2, GLuint * length){
    ...
    if(polygon1_contains_polygon2){
        ...
        *length = k;
        ...
        for(i = 0 ;  i<(current_polygon==polygon1?num1:num2) || (!another_polygon_over) ;){
            //判断条件支持动态变化，使得跳跃成了可能！！！
            //i：current_polygon的索引
            
            std::cout
            <<"Combining with "<<i<<" th"<<" vertices, total:"<<num_of_total_vertices<<'\n';
            ...
            if( (!is_in_polygon(another_polygon, another_polygon==polygon1?num1:num2, current_polygon[i]))
               &&( !is_in_array(current_array, array_length, i) ) ){//array_length对于r[]和t[]都是一样的
                //1)如果其在polygon1外部且不存在于r[]中，则压栈即可
                ...
                newpolygon[j][0] = current_polygon[i][0];
                ...
                if(current_array[p+1] != i){
                    ...
                    newpoint1 = intersection(current_polygon[i], another_polygon[m], current_polygon[(i+1<num2)?i+1:0], another_polygon[(m+1<num2)?m+1:0]);
                    show_point(newpoint1);
                    
                    p++;
                    m = another_array[p];
                    
                    newpoint2 = intersection(current_polygon[i], another_polygon[m], current_polygon[(i+1<num2)?i+1:0], another_polygon[(m+1<num2)?m+1:0]);
                    show_point(newpoint2);
                ...
                i = (current_array[p]+1) < ((current_polygon==polygon1)?num1:num2) ? current_array[p]+1 : 0;
                //既然跳到另一个多边形上，i也需要改变.如果跳变前我方数组是r，则i变成对方数组里的t[i+1]
                
                if(j == num_of_total_vertices ){
                    break;
                }
